...
for index, stocksrow in t5stocks.iterrows():
    ...
    model = TFTForecaster(input_size=1,hidden_size=hidsize,num_heads=numheads,num_encoder_layers=numencods,output_size=forecasthorz,dropout=dr,learning_rate=lr)
    # trainmodel
    ...
    metrics = evalFor(acts_flat, predictions_flat, "TFT")
...
for index, (tik, rest) in enumerate():
    ...
    ax.set_title(f'{tik}-TFT Forecast RMSE: ${rest["metrics"]["rmse"]} | MAPE: {rest["metrics"]["mape"]}%', fontweight='bold')
